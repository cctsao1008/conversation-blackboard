...
mod tests {
    ...
    fn event_count(conn: &Connection) -> i64 {
        conn.query_row("SELECT COUNT(*) FROM authorization_admin_events", [], |row| row.get(0))
            .unwrap()
    }
    ...
    fn invalid_request_creates_neither_grant_nor_event() {
        ...
        assert!(create_durable_grant(&conn, &actor, &request).is_err());
        assert_eq!(event_count(&conn), 0);
        let grants: i64 = conn.query_row("SELECT COUNT(*) FROM principal_grants", [], |row| row.get(0)).unwrap();
        assert_eq!(grants, 0);
    }
}
'''
Path("src/authorization_admin.rs").write_text(module, encoding="utf-8")

main = Path("src/main.rs").read_text(encoding="utf-8")
main = replace_once(main, "mod authorization;\n", "mod authorization;\nmod authorization_admin;\n", "main module")
Path("src/main.rs").write_text(main, encoding="utf-8")

schema = Path("schema.sql").read_text(encoding="utf-8")
schema_anchor = """CREATE INDEX IF NOT EXISTS idx_delegated_grants_lookup
ON delegated_grants(
    principal_provider,
    principal_subject,
    participant_id,
...
ON authorization_admin_events(grant_store, grant_id, id);
"""
schema = replace_once(schema, schema_anchor, schema_add, "schema admin provenance")
Path("schema.sql").write_text(schema, encoding="utf-8")

db = Path("src/db.rs").read_text(encoding="utf-8")
db = replace_once(
    db,
    "use crate::{\n    execution,\n",
...
grant = replace_between(
    ...
    "",
    "remove durable CLI mutation model",
)
grant = replace_between(
    grant,
...
grant = replace_between(
    ...
    "fn normalize(",
    "fn normalize(",
    "remove CLI participant mutation validator",
)
Path("src/grant_admin.rs").write_text(grant, encoding="utf-8")
