...
Path("src/web_auth.rs").write_text(r'''use std::{
...
pub fn credential_bundle(participant_id: &str, private_key: &str) -> String {
    ...
    format!("{CREDENTIAL_BUNDLE_PREFIX}{}", URL_SAFE_NO_PAD.encode(bytes))
}
...
pub fn resolve_request_identity_for_target(
    ...
    let signed_attempt = headers.contains_key(SIGNATURE_HEADER)
...
replace_once(
    ...
    r'''async fn post_signed_message(
    state: &AppState,
...
replace_once(
    ...
    r'''async fn require_identity_for_target(
    state: &AppState,
    headers: &HeaderMap,
    method: &'static str,
    request_target: &str,
    ...
    let identity = with_db(state, move |conn| {
        request_auth::resolve_request_identity_for_target(
            conn,
            &headers,
            method,
            &request_target,
...
replace_once(
    "web/index.html",
    """      <label for="private-key">Participant credential</label>
      <div class="connect-row">
        <input id="private-key" type="password" autocomplete="off" spellcheck="false" aria-label="Private key" placeholder="Private key">
        <button id="connect" type="button">Connect</button>
      </div>
      <p class="hint">Private key is kept only in page memory and is not stored in browser storage.</p>
""",
    """      <label for="participant-credential">Participant credential</label>
      <div class="connect-row">
        <input id="participant-credential" type="password" autocomplete="off" spellcheck="false" aria-label="Participant credential" placeholder="bbcred-v1:…">
        <button id="connect" type="button">Connect</button>
      </div>
      <p class="hint">Credential is parsed locally. The private signing key is never sent or stored in browser storage.</p>
""",
)
...
'''
new_api = r'''  function base64urlDecode(value) {
...
if old_api not in text:
    raise SystemExit("old api block not found")
...
'''
new_connect = r'''  async function connect() {
    $("connect-error").textContent = "";
...
if old_connect not in text:
    raise SystemExit("old connect block not found")
...
'''
new_post = '''      const payload = await signedWriteEnvelope(
        state.channel,
        $("kind").value,
        body,
...
if old_post not in text:
    raise SystemExit("old post block not found")
...
if "X-Blackboard-Private-Key" in text or "state.privateKey" in text:
    raise SystemExit("raw private-key transport still present in browser app")
app.write_text(text)

Path("src/web_auth_contract_tests.rs").write_text(r'''use std::path::PathBuf;
...
async fn json_request(
    ...
    let response = router.clone().oneshot(builder.body(request_body).unwrap()).await.unwrap();
...
async fn signed_browser_reads_bind_exact_request_target() {
    ...
    let signature = signed_auth::sign_message_signature(&fixture.private_key, &canonical).unwrap();
    let headers = [
        ("X-Blackboard-Participant-Id", fixture.participant_id.clone()),
        (
