...
AUTH = r'''use rusqlite::{params, Connection, OptionalExtension, Transaction};
use serde::Serialize;
...
pub fn authorize_for_intent(
    ...
    Ok(if decision.allowed {
        IntentAuthorization::Allowed {
            consume_grant_id: decision.consume_grant_id,
        }
    } else {
...
fn implicit_authority(
    ...
    implicit_authority_reason(principal, participant_id, capability, participant).is_some()
}
...
mod tests {
    ...
    fn consumed_one_shot_distinguishes_replay_from_broken_consumption() {
        ...
        assert_eq!(broken.reason, "delegated_consumed_without_committed_receipt");
...
Path("src/authorization.rs").write_text(AUTH, encoding="utf-8")

p = Path("src/grant_admin.rs")
s = p.read_text(encoding="utf-8")
s = s.replace("    authorization::{self, IntentAuthorization},\n", "    authorization,\n")
s = re.sub(
...
s = s.replace(
    ...
    'println!("consume_on_commit : {}", explanation.consume_grant_id.is_some());'
)
start = s.index("\nfn explain_grant(")
end = s.index("\nfn deactivate_grant(", start)
s = s[:start] + s[end:]
s = s.replace("let explanation = explain_grant(", "let explanation = authorization::evaluate_authorization(")
p.write_text(s, encoding="utf-8")

p = Path("src/execution.rs")
s = p.read_text(encoding="utf-8")
old = '''    let consume_grant_id = match authorization::authorize_for_intent(
        &tx,
...
new = '''    let authorization_decision = authorization::evaluate_authorization(
        &tx,
        &request.authority.principal,
        &request.intent.participant_id,
        &request.intent.capability,
...
if old not in s:
    raise SystemExit("execution authorization block not found")
