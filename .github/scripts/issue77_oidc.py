...
Path("src/oidc.rs").write_text(r'''use std::{collections::HashMap, env, path::PathBuf, sync::Arc, time::Duration};
...
impl ApiError {
    fn unauthorized() -> Self {
        Self { status: StatusCode::UNAUTHORIZED, code: "unauthorized" }
    }

    fn forbidden() -> Self {
        Self { status: StatusCode::FORBIDDEN, code: "forbidden" }
    }

    fn bad_request(code: &'static str) -> Self {
        Self { status: StatusCode::BAD_REQUEST, code }
    }

    fn unavailable() -> Self {
        Self { status: StatusCode::SERVICE_UNAVAILABLE, code: "database_unavailable" }
    }
}
...
impl OidcState {
    pub async fn from_env(db_path: PathBuf) -> Result<Option<Self>, Box<dyn std::error::Error + Send + Sync>> {
        let issuer = env::var("BLACKBOARD_OIDC_ISSUER").ok().filter(|v| !v.trim().is_empty());
        let audience = env::var("BLACKBOARD_OIDC_AUDIENCE").ok().filter(|v| !v.trim().is_empty());
        let (Some(issuer), Some(audience)) = (issuer, audience) else {
            if env::var("BLACKBOARD_OIDC_ISSUER").is_ok() || env::var("BLACKBOARD_OIDC_AUDIENCE").is_ok() {
        ...
        let jwks_url = match env::var("BLACKBOARD_OIDC_JWKS_URL").ok().filter(|v| !v.trim().is_empty()) {
            Some(url) => {
            ...
            None => {
                ...
                let discovery = client.get(discovery_url).send().await?.error_for_status()?.json::<DiscoveryDocument>().await?;
        ...
        let jwks = client.get(jwks_url).send().await?.error_for_status()?.json::<JwkSet>().await?;
        let verifier = OidcVerifier::from_jwks(issuer, audience, jwks)?;
        Ok(Some(Self { db_path, verifier: Arc::new(verifier) }))
    }
}

impl OidcVerifier {
    fn from_jwks(issuer: String, audience: String, jwks: JwkSet) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        let mut keys = HashMap::new();
        for jwk in &jwks.keys {
            let Some(kid) = jwk.common.key_id.as_deref() else { continue; };
        ...
        Ok(Self { issuer, audience, keys: Arc::new(keys) })
    }
    ...
    fn verify(&self, token: &str) -> Result<execution::Principal, ()> {
        ...
        let subject = data.claims.get("sub").and_then(Value::as_str).filter(|v| !v.is_empty()).ok_or(())?;
...
async fn post_message(
    ...
    let participant_id = identity::validate_participant_id(&participant_id).ok_or_else(|| ApiError::bad_request("invalid_participant_id"))?;
    let token = bearer_token(&headers).ok_or_else(ApiError::unauthorized)?;
    let principal = state.verifier.verify(token).map_err(|_| ApiError::unauthorized())?;
    ...
    let channel = body.get("channel").and_then(Value::as_str).filter(|v| valid_name(v)).ok_or_else(|| ApiError::bad_request("invalid_channel"))?.to_owned();
    ...
    let message_body = body.get("body").and_then(Value::as_str).filter(|v| !v.trim().is_empty()).ok_or_else(|| ApiError::bad_request("invalid_body"))?.to_owned();
    if message_body.len() > MAX_BODY_BYTES {
        return Err(ApiError { status: StatusCode::PAYLOAD_TOO_LARGE, code: "request_too_large" });
    }
    let reply_to = match body.get("reply_to") {
        None | Some(Value::Null) => None,
        Some(Value::Number(v)) => v.as_i64().filter(|id| *id > 0).map(Some).ok_or_else(|| ApiError::bad_request("invalid_reply_to"))?,
        _ => return Err(ApiError::bad_request("invalid_reply_to")),
    };
    let intent_id = body.get("intent_id").and_then(Value::as_str).ok_or_else(|| ApiError::bad_request("invalid_intent_id"))?;
    let intent_id = execution::normalize_intent_id(intent_id).map_err(|_| ApiError::bad_request("invalid_intent_id"))?;
    ...
    let result = tokio::task::spawn_blocking(move || -> rusqlite::Result<Option<execution::MessageExecutionResult>> {
        let conn = db::connect(&db_path)?;
        let Some(writer) = identity::get_web_participant(&conn, &participant_for_db)? else { return Ok(None); };
        let request_hash = execution::message_request_hash(&channel_for_db, &kind_for_db, &body_for_db, None, reply_to);
        let authority = execution::AuthorityContext { principal: principal_for_db.clone(), mechanism: OIDC_MECHANISM.to_owned() };
        ...
        let ingress = execution::IngressProvenance {
            delivery_id: format!("oidc:{}:{}:{}", principal_for_db.subject, participant_for_db, intent_for_db),
            intent_id: intent_for_db.clone(),
            transport: "oidc-http".to_owned(),
            external_ref: intent_for_db.clone(),
            principal: principal_for_db,
        };
        execution::execute_message_intent(&conn, &writer, execution::MessageExecutionRequest {
            authority: &authority,
            ingress: &ingress,
            intent: &intent,
    ...
    }).await.map_err(|_| ApiError::unavailable())?.map_err(|_| ApiError::unavailable())?
      .ok_or_else(ApiError::forbidden)?;

    let (status, message, idempotent) = match result {
        execution::MessageExecutionResult::Created(message) => (StatusCode::CREATED, message, false),
        ...
        execution::MessageExecutionResult::IntentConflict => return Err(ApiError { status: StatusCode::CONFLICT, code: "intent_conflict" }),
        execution::MessageExecutionResult::ReplyTargetNotFound => return Err(ApiError::bad_request("reply_target_not_found")),
        execution::MessageExecutionResult::ChannelArchived => return Err(ApiError { status: StatusCode::CONFLICT, code: "channel_archived" }),
    };
    Ok((status, Json(json!({"idempotent": idempotent, "message": message}))).into_response())
}

fn bearer_token(headers: &HeaderMap) -> Option<&str> {
    let value = headers.get(header::AUTHORIZATION)?.to_str().ok()?;
    let (scheme, token) = value.split_once(' ')?;
    if !scheme.eq_ignore_ascii_case("bearer") || token.is_empty() { return None; }
    Some(token)
}

async fn read_json_object(request: Request<Body>) -> Result<Map<String, Value>, ApiError> {
    let bytes = to_bytes(request.into_body(), MAX_BODY_BYTES + 1).await.map_err(|_| ApiError::bad_request("invalid_json"))?;
    if bytes.len() > MAX_BODY_BYTES { return Err(ApiError { status: StatusCode::PAYLOAD_TOO_LARGE, code: "request_too_large" }); }
    serde_json::from_slice::<Value>(&bytes).ok().and_then(|v| v.as_object().cloned()).ok_or_else(|| ApiError::bad_request("invalid_json"))
}
...
fn valid_kind(value: &str) -> bool {
    !value.is_empty() && value.len() <= 32 && value.bytes().all(|b| b.is_ascii_alphanumeric() || matches!(b, b'-' | b'_'))
}
...
mod tests {
    ...
    fn oidc_principal_is_issuer_and_subject_and_uses_scoped_grants() {
        ...
        let verifier = OidcVerifier::from_jwks("https://issuer.example".into(), "blackboard".into(), jwks).unwrap();
        ...
        identity::provision_web_participant_identity(&conn, "agent-main", "agent", Some("Agent")).unwrap().unwrap();
        ...
        let principal = execution::Principal { provider: "oidc:https://issuer.example".into(), subject: "remote-agent-1".into() };
        assert!(authorization::authorize(&conn, &principal, "agent-main", authorization::POST_MESSAGE, Some("control-systems")).unwrap());
        assert!(!authorization::authorize(&conn, &principal, "agent-main", authorization::POST_MESSAGE, Some("blackboard-lounge")).unwrap());
    }
...
replace_once("src/main.rs", "mod model;\n", "mod model;\nmod oidc;\n")
replace_once(
    "src/runtime.rs",
...
replace_once("Cargo.toml", "clap = { version = \"4\", features = [\"derive\"] }\n", "clap = { version = \"4\", features = [\"derive\"] }\njsonwebtoken = \"9\"\n")

Path("docs/external-oidc.md").write_text(r'''# External OIDC resource-server mode
