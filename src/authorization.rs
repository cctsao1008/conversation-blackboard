...
fn validate_policy_window_request(
    ...
    AuthorizationPolicyWindowOrder,
    usize,
)> {
    let store =
        AuthorizationPolicyStore::parse(request.store).ok_or(rusqlite::Error::InvalidQuery)?;
    ...
    let limit = request
        .limit
        .unwrap_or(DEFAULT_AUTHORIZATION_POLICY_WINDOW_SIZE);
...
pub fn read_authorization_policy_window(
    ...
    let (durable_grants, delegated_grants, has_more) = match store {
        AuthorizationPolicyStore::Durable => {
            let mut grants =
                read_durable_grant_window(conn, order, request.before, request.after, fetch_limit)?;
...
mod tests {
    ...
    fn authorization_policy_window_is_bounded_and_store_selected() {
        ...
        assert_eq!(
            durable.durable_grants.len(),
            DEFAULT_AUTHORIZATION_POLICY_WINDOW_SIZE
        );
    ...
    fn authorization_policy_window_keyset_pages_without_duplicates_or_skips() {
        ...
        for index in 0..9 {
            ...
            conn.execute(
                ...
                params![
                    format!("delegated-page-{index}"),
                    format!("page-intent-{index}")
                ],
            )
            .unwrap();
        }
