...
async fn request_with_authorization(router: &Router, body: Value, authorization: &str) -> Response {
...
async fn mcp_bearer_write_uses_oidc_principal_and_matching_blackboard_grant() {
    ...
    let conn = db::connect(&fixture.db_path).unwrap();
    let audit =
        crate::execution::get_execution_audit_bundle(&conn, "single-main", "issue100-bearer-ok")
            .unwrap()
            .unwrap();
    ...
    assert_eq!(
        ...
        Some(authorization::POST_MESSAGE)
    );
...
async fn raw_bearer_token_never_enters_durable_sqlite_storage() {
    ...
    let contains_token = |bytes: &[u8]| bytes.windows(needle.len()).any(|window| window == needle);
    let db_bytes = std::fs::read(&fixture.db_path).unwrap();
    assert!(
        !contains_token(&db_bytes),
        "raw bearer token leaked into SQLite database"
    );
...
async fn bearer_delegated_one_shot_survives_failed_execution_and_replays_idempotently() {
    ...
    let succeeded =
        request_with_authorization(&router, success_body.clone(), &format!("Bearer {token}")).await;
    ...
    let replayed =
        request_with_authorization(&router, success_body, &format!("Bearer {token}")).await;
